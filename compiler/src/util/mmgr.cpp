...
// !!IMPORTANT!!
//
// This software is self-documented with periodic comments. Before you start using this
// software, perform a search for the string "-DOC-" to locate pertinent information about
...
bool	&m_breakOnRealloc(void *reportedAddress)
{
	...
	assert(au->allocationType == m_alloc_malloc ||
...
bool	m_validateAllocUnit(const sAllocUnit *allocUnit)
{
	...
	for (unsigned int i = 0; i < paddingSize; i++, pre++, post++)
	{
		...
		assert(*pre == static_cast<long>(prefixPattern));
		...
		assert(*post == static_cast<long>(postfixPattern));
	}
